...
if (!(Test-Path C:\Users\packer\windows.iso))
{
    Write-Error "windows.iso not found"
    Exit 1
}
...
if (Get-Command "Mount-DiskImage" -ErrorAction SilentlyContinue)
{
    ...
    $mountResult = Mount-DiskImage -ImagePath C:\Users\Packer\windows.iso -PassThru
...
else
{
    ...
    & "$($ENV:SystemRoot)\temp\wincdemu.exe" C:\Users\Packer\windows.iso V:
    $driveLetter = "V"
}
...
if ($ENV:PACKER_BUILDER_TYPE -eq "virtualbox-iso")
{
    ...
    #deal with PowerShells foolishness around executing external commands
    write-output "$($driveLetter):\VBoxWindowsAdditions.exe /S" | Out-File c:\vmtools.bat -Encoding ascii
    & c:\vmtools.bat
}
...
{
    Write-Output "Use powershell"
    Dismount-DiskImage -ImagePath "C:\users\packer\windows.iso"
}
else
{
    Write-Output "Use WinCDEmu"
    & "$($ENV:SystemRoot)\temp\wincdemu.exe" /umountalll
